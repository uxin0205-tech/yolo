...
def test_early_stop_requires_normal_completion_marker(tmp_path: Path) -> None:
    ...
    assert completed_training_checkpoint(run, expected_epochs=8) == (
        run / "ultralytics/weights/best.pt"
    ).resolve()


def test_long_phase_patience_is_valid() -> None:
    recipe = TrainingRecipe(
        phase="phase-c", parent="phase-b-gate", trainable_scope="attention_refinement",
        weights="parent.pt", data="data.yaml", epochs=24, batch=16, imgsz=640, device="0",
        workers=8, seed=2, patience=8, optimizer="AdamW", lr0=2.5e-6,
        scheduler="constant", lrf=1.0, warmup_epochs=0.0, warmup_bias_lr=0.0,
        weight_decay=0.0005, selection_metric="metrics/mAP50-95(B)", amp=True,
        deterministic=True,
    )
