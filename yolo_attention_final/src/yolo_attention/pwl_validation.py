    ...
    def update(
        ...
        *,
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            float_cosine = F.cosine_similarity(exact_vectors, float_vectors, dim=-1, eps=1e-12)
...
class PWLValidationAccumulator:
    ...
    def summary(self) -> dict[str, object]:
        return {
            ...
            "heads": [{"head": head, **state.summary()} for head, state in enumerate(self._heads)],
        }
...
class PWLModelDiagnosticsCollector:
    ...
    def __init__(
        ...
        *,
        ...
        if set(modules) != set(expected_paths):
            raise ValueError(f"expected PWL diagnostic paths {list(expected_paths)}, found {sorted(modules)}")
        required = (
            BasisKind.HADAMARD,
    ...
    def histogram_rows(self) -> list[dict[str, float | int | str]]:
        return [row for path, _ in self._modules for row in self._accumulators[path].histogram_rows()]
