...
production package 的公開入口是 `python -m yolo_attention.cli`。各模組維持單一責任：設定在 `config.py`，數學方法在 basis/bias/normalization/BDCN 模組，兩處模型轉換在 `integration.py`，PWL streaming diagnostics 與正式報告分別在 `pwl_validation.py`／`pwl_experiment.py`，訓練與 queue side effect 只在 runner/backend/executor。
