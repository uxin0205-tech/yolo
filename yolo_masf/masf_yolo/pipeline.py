...
from masf_yolo.training.resume import execute_with_retries
...
CANONICAL_TRAINING_STAGES = ("b1_a", "b1_b", "formal_m0", "formal_m1", "formal_m2", "formal_m3")
...
class FormalPipeline:
    ...
    def _batch_probe(self) -> StageResult:
        ...
        atomic_write_json(path, {"batch": batch, "variants": list(VARIANTS)})
    ...
    def _train(self, stage: str, variant_id: str) -> StageResult:
        ...
        model, transfer = self._initial_model(stage, variant_id)
        profile = self._profile_for_stage(stage, variant_id)
        atomic_write_json(stage_root / "resolved_args.json", _jsonable(profile))
        expected_last = Path(profile["project"]) / profile["name"] / "weights" / "last.pt"

        def operation(_attempt: int, _resume: bool):
            resume_path = expected_last if expected_last.is_file() else None
            return run_training(model, profile, resume_path=resume_path)
        ...
        trained = YOLO(str(result.best), task="detect").model.float()
        trained.masf_variant = variant_id
        trained.masf_variant_hash = get_variant(variant_id).config_hash
        manifest = save_canonical_checkpoint(
            trained,
            canonical,
            get_variant(variant_id),
        ...
        record = {
            ...
            "canonical_hash": manifest.checkpoint_hash,
        ...
        return StageResult({"run": sha256_file(path), "canonical": manifest.checkpoint_hash})
    ...
    def _profile_variant(self, variant_id: str, output_root: Path) -> HardwareProfile:
        ...
        profile = profile_module(model, torch.zeros(1, 3, 640, 640))
