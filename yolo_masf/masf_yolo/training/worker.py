...
from masf_yolo.models.transfer import transfer_b1_canonical, transfer_sp2p_parents
...
def _initial_model(request: TrainingWorkerRequest) -> tuple[torch.nn.Module | None, dict[str, Any] | None]:
    ...
    if request.stage == "b1_b":
        ...
        return model, None
    if request.stage in {"sp2_b", "sp2p_b"}:
        from ultralytics import YOLO
        from masf_yolo.variants import get_variant

        parent_stage = "sp2_a" if request.stage == "sp2_b" else "sp2p_a"
        model = YOLO(
            _read_training_record(artifact_root, parent_stage)["best"],
            task="detect",
        ).model
        model.masf_variant = request.variant_id
        model.masf_variant_hash = get_variant(request.variant_id).config_hash
        return model, None
    if request.stage in {"smoke_sp2p", "sp2p_a"}:
        from masf_yolo.variants import sp2p_variant_id

        selection_path = artifact_root / "selection.json"
        selection = json.loads(selection_path.read_text(encoding="utf-8"))
        selected_partial = selection.get("selected")
        architecture_variant = sp2p_variant_id(selected_partial)
        if request.variant_id != architecture_variant:
            raise ValueError(
                f"SP2P worker variant {request.variant_id} does not match selection {selected_partial}"
            )
        sp2 = build_model(
            "SP2",
            checkpoint=artifact_root / "training" / "sp2_b" / "canonical.pt",
        )
        partial = build_model(
            selected_partial,
            checkpoint=(
                artifact_root
                / "training"
                / f"formal_{selected_partial.lower()}"
                / "canonical.pt"
            ),
        )
        model = build_model(architecture_variant)
        transfer = transfer_sp2p_parents(
            model,
            sp2.state_dict(),
            partial.state_dict(),
