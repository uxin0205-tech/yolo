...
variants: [B1, M7, M0, M1, M2, M3, P3M, SP2]
environment: {python: '3.14', torch: '2.11.0+cu128', cuda: '12.8', ultralytics: '8.4.90', faster_coco_eval: '1.7.2', onnx: '1.22.0', ml_dtypes: '0.5.4', protobuf: '7.35.1'}
dataset: {source: dataset, split_ratios: [0.8, 0.1, 0.1], class_names: [ball, bat], seed: 42, minimum_ball_count: 50}
model: {source_weights: weights.pt, source_weights_sha256: a, official_model_yaml_sha256: b, official_default_yaml_sha256: c, imgsz: 640, nc: 2, scale: m, sp2_hidden_channels: 32, sp2_auxiliary_loss_weight: 1.0}
...
profiling: {precision: fp16, batch: 1, warmup: 100, iterations: 1000}
pipeline: {max_attempts: 3, device: 0, systemd_unit_prefix: masf-yolo-phase1, wait_for_units: [yolo-p2-study.service]}
""".strip()
    )
    ...
    assert "All metrics are data-exposed operational ablations" in text
    assert "formal_m7: pending" in text
    assert "sp2_a: pending" in text
    assert "sp2_b: pending" in text
    assert "sp2p_a: pending" in text
    assert "sp2p_b: pending" in text
