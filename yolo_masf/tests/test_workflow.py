...
def test_phase1_dag_has_selection_before_test_and_no_phase2_nodes() -> None:
    ...
    assert names[names.index("verify") + 1] == "gpu_wait"
    assert names[names.index("gpu_wait") + 1] == "preflight"
...
def test_sp2_and_sp2p_stages_have_locked_order_without_onnx() -> None:
    ...
    assert names.index("formal_m3") < names.index("formal_p3m")
    ...
    assert names.index("sp2p_b") < names.index("baseline_b0")
