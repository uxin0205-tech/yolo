...
def test_repository_resume_discards_cpu_cached_criterion() -> None:
    model = nn.Sequential(nn.Conv2d(3, 4, 1))
    model.masf_variant = "M2"
    model.args = {}
    model.criterion = SimpleNamespace(proj=torch.arange(16, device="cpu"))
    trainer = RepositoryDetectionTrainer.__new__(RepositoryDetectionTrainer)
    trainer.args = SimpleNamespace()

    resumed = trainer.get_model(weights=model)

    assert resumed is model
    assert resumed.criterion is None
