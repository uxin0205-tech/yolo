...
FORMAL_PHASE_C_BATCH = 8
FORMAL_NBS = 16
FORMAL_VALIDATION_BATCH = 8
...
class CommonTrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 < self.fraction <= 1.0:
            raise ValueError("fraction 必須介於 0（不含）與 1（含）之間")
...
def training_config_for_phase(
    common: CommonTrainingConfig,
    phase_name: str,
) -> CommonTrainingConfig:
    """套用正式 phase 專屬 batch；A1/A2/B 維持 common，只有 C 使用 8×2。"""

    if phase_name != "c":
        return common
    return replace(
        common,
        batch=FORMAL_PHASE_C_BATCH,
        nbs=FORMAL_NBS,
