# T1 BinaryAttention run

- 目的：YOLO11m BinaryAttention accuracy/fidelity experiment。
- 相對前一 variant：`T0`；本 run 改動為 `SignOnlyBinaryAttention` / `sign`。
...
- config hash：`be9b62613a9341cc51dd8352dc3af7e1353414f413660b2123c7145f11803940`。

## Attention / loss

- QK：`sign`；QAT：`True`；bias：`none`。
- quantization contract：`{"bias_initialization": "none", "bias_parameterization": "none", "p8_scale": "none", "qk_scale": "none", "v8_scale": "none"}`。
- KD：`None`，components=`[]`。
