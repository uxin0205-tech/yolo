...
保留整理前文字供追溯；其中「目前」「現行」只代表各段寫作當時。請回到[現行首頁](../../README.md)。命令仍以專案根目錄執行，不應據此重啟舊 queue。

比較口徑：[歷史與本輪證據層級釐清](../../docs/reports/2026-09-07-evidence-consistency-boundaries.md)。歷史 1,332 筆是權重投影誤差；本輪 592 組是輸出探測，兩者都不是逐層 mAP。現行 40 組區域 PTQ 與 6 份 QAT 計畫已核對 V36 同源血緣。

新增實測：[同 parent 五回合 QAT 恢復結果](../../docs/reports/2026-09-07-continuous-qat-recovery-results.md)。Fixed-SD4 與 LS-SD4 均完成五回合，各回合搜尋雙門檻通過；queue 自動接續 MASF exact ternary，不代表全部實驗或部署驗證完成。

判讀提醒：[PTQ未達標不等於QAT無法恢復](../reports/2026-09-08-quantization-phase-handoff.md)。PTQ與QAT分開列結果，最終仍須通過總精度門檻。

目前接續：[六組短 QAT 與恢復候選](../../docs/worklogs/2026-09-07-six-short-qat-continuation.md)。40 格同 parent 十區 PTQ、592 格單層探測已完成；六組五回合 QAT 已建立串行 queue，每組通過 CPU 真實圖預檢後才訓練。狀態檔為 `artifacts/queues/full-model-continuous-0907/execution-status.json`。PTQ 未達標仍保留有限恢復實驗；已完成 QAT 結果見上方報告，不預判尚未完成候選。

已通過前置：[P0工作紀錄](../../docs/worklogs/2026-09-07-continuous-p0-implementation.md)。共同 parent 重載、搜尋精度重驗與分布分析已有產物；勿將下方歷史 planned 狀態當成最新進度。

最新執行規劃：[5epochs／持續批次／blocking monitor](../CURRENT_PLAN.md)，取代四天硬截止與3epoch上限；本批入口 `scripts/run_continuous_qat_queue.py`。歷史實驗數字不回寫。
...
> 2026-09-07最新盤點：[全模型量化報告與四天計畫](reports/2026-09-07-full-model-audit-four-day-plan.md)／[數據與圖表](../../deliverables/full-model-audit-2026-09-07/README.md)。V36已完成3 epochs，148部署權重路徑均fake-quant；epoch1/2搜尋雙門檻通過。完整整數部署及十區独立accuracy sensitivity仍待完成。後續四天DAG尚未排入live queue。下列2026-09-05及更早進度保留為歷史。
...
- [新增整合roadmap](../../configs/experiments/full35-integrated-quantization-roadmap-v1.yaml)／[Paper-TWN逐區規劃](reports/2026-09-04-integrated-quantization-and-paper-twn-plan.md)：不改舊結果，將目前V19、W8/mixed、SD4/LS-SD4、ternary、activation coupling與formal串成backbone→neck→head流程。
- [V29七組來源與qSiLU接手設計](reports/2026-09-05-v29-selection-and-qsilu-handoff-plan.md)：說明七組短QAT的PTQ/re-gate依據、第一組負結果、qSiLU Q0–Q7及regional Hardswish條件旁支。
- [V19／V29逐區量化執行報告](reports/2026-09-04-v19-v29-progressive-quantization-execution.md)：整合歷史activation與bit結果、V19鎖定parent、148×5 CPU profile、V29 PTQ、recovery gate修正、7-job QAT queue與後續停止線。
...
- [poly_shift PTQ、特殊格式與QAT入口總報告](reports/2026-09-03-poly-shift-ptq-special-format-and-qat-entry.md)：W8–W4、mixed-bit、SD4／ternary、整體雙指標與剩餘工作。
- [V5 mAP50與bit邊界報告](reports/2026-09-03-v5-map50-bit-boundary-search.md)：`-0.015`總門檻、W8 re-gate、W7–W4完整結果與後續路由。
...
- [CPU P0詳細報告](reports/2026-09-01-cpu-p0-integer-exact-routing.md)：integer boundary、三parent exact矩陣、routing v3結果與GPU停止線。
...
- [架構與實驗稽核報告](reports/2026-09-01-architecture-experiment-audit.md)：已修問題、現行矩陣、GPU前blocker與研究方向。
...
- [Hardswish policy修訂報告](reports/2026-08-31-hardswish-policy-revision.md)：uniform／regional證據、active矩陣、CPU結果與停止線。
...
- [歷史V0–V3 CPU報告](reports/2026-08-30-v1-v3-cpu-implementation.md)／[delivery v1](../../artifacts/manifests/v1-v3-cpu-delivery-v1.yaml)：保留含`poly_quality`的原始證據，不再是active matrix。
...
- [Weight region重新規劃](../reports/2026-09-08-quantization-method-and-decisions.md)：A3至A8判定、真實region盤點、INT8／INT4矩陣、指標與QAT方法。
- [Weight region歷史v1計畫](../../configs/experiments/weight-region-sensitivity-plan-v1.yaml)：保留舊結果血緣，不是現行執行入口。
- [backbone_early PTQ結果報告](reports/2026-08-29-weight-ptq-backbone-early-v1.md)：W8／W4六格scorecard、容量、限制與下一步。
