"""準備 0914 本輪完成結果的發布副本；白名單、不刪檔、不啟動 GPU。"""
import argparse
import ast
import hashlib
import json
...
FILES=['README.md','experiments/README.md','reports/current-model/README.md',
 'reports/checkpoints/README.md','docs/worklogs/README.md','tools/publish_update_0914.py']
TREES=['experiments/pose_masf_training_v1','experiments/post_masf_hardware_v1',
       'experiments/post_binary_rep_v1','reports/update-0914']
LINK=re.compile(r'(!?\[[^\]\n]+\])\((<[^>]+>|[^)\n]+)\)')
ALLOWED={'.md','.py','.json','.jsonl','.csv','.dot','.svg','.png'}

def selected():
    sources={ROOT/p for p in FILES}
    sources.update(p for p in (ROOT/'docs/worklogs').glob('2026-09-1[34]*.md'))
    for folder in TREES:
        for p in (ROOT/folder).rglob('*'):
            rel=p.relative_to(ROOT/folder)
            if any(x in rel.parts for x in ('datasets','__pycache__')):continue
            if p.name in ('publication-check.json','PUBLISHED.md'):continue
...
def main():
    ...
    for src,dst in mapping.items():
        ...
        if ext=='.json':
            ...
            counts['json']+=1
        if ext=='.jsonl':
            for line in dst.read_text().splitlines():json.loads(line,parse_constant=invalid)
        if ext=='.csv':
            import csv
            rows=list(csv.reader(dst.open()));assert rows and all(len(row)==len(rows[0]) for row in rows)
        ...
        if ext=='.md':
            ...
            for m in LINK.finditer(dst.read_text()):
                ...
                counts['links']+=1
    result={'status':'passed','commit_subject':'5090 Uodate 0914','scope':'MASF B5、BinaryQK E5、Rep 三組完成報告、程式、設定、數值證據與權重 SHA；不含大型權重',
    ...
    for base in (ROOT,wt/'yolo_optimize'):
        edit(base/'reports/update-0914/publication-check.json',json.dumps(result,ensure_ascii=False,indent=2))
