...
結果根目錄：`/home/uxin/yolo/yolo_optimize/artifacts/direction1-20260908`。實際超參數見各 run 的 `resolved-config.json`；逐步進度見 `progress.jsonl`；GPU 每 600 秒監測見 `logs/*.monitor.jsonl`。
