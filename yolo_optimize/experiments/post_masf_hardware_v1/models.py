"""MASF B5 後的硬體候選；保留共同來源，無未改架構加訓對照。"""
import copy
...
PARENT=b.RUN/'epochs/e5/ema.pt'
PARENT_SHA='1b46fca17f4ae414f622e39afb6d4bae8686ac3a760e23b65fd971500a39e2c7'
ARMS=('scale_bias','rep17','rep20')
...
class Source(b.TrainingSource):
    def __init__(self,*args,arm='native_qk_reference',**kwargs):
    ...
    def provenance(self,kind='float'):
        return {**super().provenance(kind),'experiment':'post_masf_hardware_v1','arm':self.variant,
            ...
            'bias':'decomposed relative position signed 16-bit m/1024; initialized from historical selected BinaryQK bias',
            'deployment_claim':'software reference, no target-board latency/energy measured'}

def build(arm):
    assert arm in (*ARMS,'native_qk_reference')
    assert b.sha256(PARENT)==PARENT_SHA
    model,_=b.build();model.load_state_dict(torch.load(PARENT,map_location='cpu',weights_only=True)['state_dict'],strict=True)
    if arm=='scale_bias':
        template=SelectedSource().build_task_models('float').detect
        replace_attention(model.graph,template)
    elif arm.startswith('rep'):
        index=int(arm[3:]);old=model.graph.model[index]
...
def configure(model,arm):
    for n,p in model.named_parameters():
        ...
        special=('.attn.' in n) if arm=='scale_bias' else n.startswith(f'graph.model.{int(arm[3:])}.') if arm.startswith('rep') else False
        p.requires_grad_(arm!='native_qk_reference' and (head or special))
    model.train()
    ...
    if arm=='native_qk_reference':model.eval()
