...
固定 parent：`/home/uxin/yolo/yolo_optimize/experiments/pose_masf_priority_v1/artifacts/parent/native-qk-e2-inference.pt`。SHA-256：`4257ca9c471736aa97d0077097526b7863a9268b773913119b9969991d00b0f3`。原完整 E2 續訓檔與 optimizer／scheduler／scaler／RNG 均保留，可從 E3 接續。
...
| --- | ---: | ---: | ---: |
| COCO overall 框 | 50.2215 | 50.2215 | +0.0000 |
| COCO person 框 | 62.4625 | 62.4625 | +0.0000 |
| BBAT overall 框 | 60.8301 | 60.8219 | -0.0082 |
| BBAT overall 關鍵點 | 88.6944 | 88.6935 | -0.0008 |
| ball 框 | 49.0438 | 49.0286 | -0.0151 |
| ball 關鍵點 | 84.9048 | 84.9518 | +0.0470 |
| bat 框 | 72.6164 | 72.6152 | -0.0012 |
| bat 關鍵點 | 92.4839 | 92.4353 | -0.0487 |
...
- α = 0.017398657。
- 全資料聚合的 `||MASF(P3)−P3||₂ / ||P3||₂` = 2.1409%。
- 逐圖中位數 = 2.1682%，P95 = 2.3485%，最大值 = 2.5419%。
...
| --- | ---: | ---: | --- |
| Params | 26,528,689 | 26,604,466 | 完整共享雙 head，增加 75,777（0.286%） |
| Model size | 101.855 MiB | 102.188 MiB | 實際 inference-only 檔，包含序列化差異 |
| MAC（估算） | 47.923712 G | 48.398848 G | Conv／Linear／QK／AV |
| FLOPs（估算） | 95.847424 G | 96.797696 G | 2 FLOPs/MAC |
| Peak memory | 473.931 MiB | 474.712 MiB | GPU allocated，非訓練記憶體 |
| CPU latency | 668.649 ms | 681.408 ms | 模型推論中位數，4 threads |
| GPU latency | 21.639 ms | 22.036 ms | RTX 5090 模型推論中位數 |
| target hardware latency | 未量測 | 未量測 | 尚無專用目標板實測 |
| energy/frame | 未量測 | 未量測 | 不用 TDP 或訓練功率代替 |

量測：Intel(R) Core(TM) Ultra 9 285K、NVIDIA GeForce RTX 5090，PyTorch 2.11.0+cu128，batch 1、640×640、FP32、BitTrue PWL，`task=both` 一次共享 trunk＋兩個 head。CPU warmup 5、正式 20 次；GPU warmup 20、正式 100 次，CUDA events 計時。

CPU P95：689.891 → 685.898 ms；GPU P95：22.662 → 22.430 ms。本次中位數稍慢，但 P95 並非同方向；只有單次依序量測，不能把約 2% 的差距當成統計顯著。
...
- [固定來源](<source-pin.json>)、固定 E2 推論權重（本機保存：`artifacts/parent/native-qk-e2-inference.pt`；本次未上傳）。
- Pose MASF 候選權重（本機保存：`artifacts/comparison-v1/pose_masf/pose-masf-transferred-inference.pt`；本次未上傳）。
- 候選 SHA-256：`ebd1cbb13bcf2c47f31d2fc650e4eeb81ce2cf0101e5b9544047016d749f21d2`。它是 inference-only，不含新的訓練 optimizer，也不是單靠官方 YOLO(path) 可直接辨識的任意 pickle。
...
以上保留移接比較結案時的實測與配對建議。使用者後續取消 A 組加訓；僅 B 組程式與 CPU 檢查已完成，GPU 與等待佇列未啟動。現行設定、完整架構圖及歸因限制見 [B 組專項](<../pose_masf_training_v1/README.md>)。本次依明確指示發布現有圖與分析，commit 名稱為 `5090 Done 0913`。
