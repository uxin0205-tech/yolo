"""BinaryQK E5 後的三個 RepConv 候選；不加訓 control。"""
import argparse
import copy
...
BASE=json.loads((PARENT.parents[2]/'summary.json').read_text())['independent_validation']['bittrue']
MASF_BASE=json.loads((b.RUN/'analysis/summary.json').read_text())['e5']
...
def optimizer_for(model,arm):
    ...
    for n,p in model.named_parameters():
        ...
        if any(n.startswith(f'graph.model.{i}.') for i in LAYERS[arm]):role='rep'
...
def report(out,arm,metrics,verification):
    ...
    gate = gate and all(end[k]>=MASF_BASE[k]-.001 for k in keys)
    ...
    for k in keys:lines.append(f'| {k} | {100*MASF_BASE[k]:.4f} | {100*BASE[k]:.4f} | {100*end[k]:.4f} | {100*(end[k]-BASE[k]):+.4f} |')
    lines+=['',f'工程参考門檻（同時保護 BinaryQK E5 與原 MASF B5）：{"通過，僅列候選" if gate else "未通過，不建議取代"}。未自動升版；不增加 epoch。',
...
def main():
    ...
    if args.phase=='smoke':
        def hook(opt,a,k):
            ...
            if args.arm=='scale_bias':
                ...
                for index in LAYERS[args.arm]:
                    assert grads.get(f'graph.model.{index}.conv2.bn.weight',0)>0,f'layer{index} 新支路沒有 task gradient'
    ...
    for epoch in range(start,1 if args.phase=='smoke' else 5):
        ...
        if args.phase=='smoke':
            ...
            for index in LAYERS[args.arm]:
                assert changes[f'graph.model.{index}.conv2.bn.weight']>0,f'layer{index} 新支路未更新'
            if args.arm=='scale_bias':
                for site in SITES:
    ...
    if args.arm.startswith('rep'):model,_=build('parent_reference')
