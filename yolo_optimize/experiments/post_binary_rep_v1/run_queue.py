...
def retire_old(emit=None):
    import signal
    transition=json.loads((HERE/'transition.json').read_text())
    pid=transition['retiring_child'];stat=Path(f'/proc/{pid}/stat');train=Path(transition['old_train'])
    def same():
        if not stat.exists():return False
        try:return stat.read_text().split(') ',1)[1].split()[19]==transition['retiring_child_start_ticks']
        except FileNotFoundError:return False
    def record(status,**details):
        row={'status':status,'time':datetime.now(timezone.utc).isoformat(),'old_child':pid,**details}
        with (ART/'transition-events.jsonl').open('a') as f:f.write(json.dumps(row,ensure_ascii=False)+'\n')
        print(json.dumps(row,ensure_ascii=False),flush=True)
    if same():
        record('WAITING_OLD_EPOCH_BOUNDARY')
        while same():
            completed=[]
            for metric in sorted((train/'epochs').glob('e*/metrics.json')):
                epoch=int(metric.parent.name[1:])
                try:valid=json.loads(metric.read_text())
                except json.JSONDecodeError:continue
                if valid and (train/f'checkpoints/epoch-{epoch:02d}.pt').exists():completed.append(epoch)
            if completed:
                # 再核對 PID 身分，只終止已被新規格取代的該 child；保留完整回合與驗證。
                if same():os.kill(pid,signal.SIGTERM)
                record('OLD_JOB_RETIRED',completed_epochs=completed,all_artifacts_preserved=True)
                break
            time.sleep(60)
        while same():
            try:
            ...
            time.sleep(1)
    else:record('OLD_JOB_ALREADY_EXITED')

def main():
    parser=argparse.ArgumentParser();parser.add_argument('--execute',action='store_true');args=parser.parse_args()
    if not args.execute:print('READY：BinaryQK E5 → Rep17 → Rep20 → Rep17＋20，皆 smoke→5E→驗證分析；原生QK重用既有結果');return 0
    ...
    base=json.loads((HERE.parent/'post_masf_hardware_v1/artifacts/scale_bias/train/summary.json').read_text())['independent_validation']['bittrue']
    summaries={a:json.loads((HERE/'artifacts'/a/'train/summary.json').read_text()) for a in plan['arms']}
    lines=['# BinaryQK E5 之後：三個 RepConv 候選比較', '',
