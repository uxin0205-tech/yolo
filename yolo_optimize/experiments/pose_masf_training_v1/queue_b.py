...
def wait_interval():
    ...
    while time.monotonic()<until:time.sleep(min(60,until-time.monotonic()))
...
def main():
    ...
    for job,result in jobs:
        ...
        with log.open('x') as f:
            child=subprocess.Popen([sys.executable,'-B',str(HERE/'training_b.py'),job],stdout=f,stderr=subprocess.STDOUT,
