...
class BDCNNormalizer(nn.Module):
    ...
    def _weights(self, scores: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        self.last_bucket_histogram = torch.bincount(bucket.reshape(-1), minlength=self.bank.levels).detach()
        self.last_bucket_overflow_rate = (
            (distance > self.step * (self.bank.levels - 1)).float().mean().detach()
        )
        ...
        self.diagnostic_distance_max = (
            maximum if self.diagnostic_distance_max is None else max(self.diagnostic_distance_max, maximum)
        )
