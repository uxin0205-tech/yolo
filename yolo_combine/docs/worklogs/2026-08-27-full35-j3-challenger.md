...
- J3已改善固定joint score並通過八項gate，因此升格；改善只有`+0.0011365230`，
  不能誇大為全面提升。ball box與ball pose相對J2仍小幅下降，完整比較由最終報告
  同時呈現。
...
- 正式b32 service：`yolo-full35-v2-j3-b32-seed0.service`已成功結束，Result=success。
- queue state：`variants/full35/artifacts/queue/full35-v2-j3-b32-seed0/queue-status.json`。
- 實跑11/20 epochs；global epoch58（J3 local epoch5，zero-based）為最佳點。
- 最佳Bit-True joint score `0.7111747389752653`，較J2 `+0.0011365230476836`。
- 八項gate全部通過；最差delta為ball pose `-0.0163543649205753`。
- global epoch63因連續5次未改善而依patience5正常early stop，不是中斷。
- 獨立final evaluation已完成Float與Bit-True；queue status為`completed`，
  `candidate_improved_parent_best=true`。
